...
`load_reference()` and `describe()` are exported: wake/make_samples.py stamps the
same verb3/verb5 numbers into every committed sample header. The repo does not
distribute the Wake, so a committed piece of generated text needs the "is this
just quoting Joyce?" question answerable with a number rather than a shrug — and
that number has to come from THIS measure, not a second copy of it that drifts.
"""
...
def load_reference(path):
    """Word set + 3/5-gram sets for a corpus, the thing `describe` scores against."""
    text = pathlib.Path(path).read_text(encoding="utf-8").lower()
    words = re.findall(r"[a-z']+", text)
    return {
        "path": str(path),
        "vocab": set(words),
        "ng": {n: {tuple(words[i:i + n]) for i in range(len(words) - n + 1)} for n in (3, 5)},
    }


def describe(text, ref, vocab=None):
    ...
    vocab = ref["vocab"] if vocab is None else vocab
    ...
    for n in (3, 5):
        g = ngrams(w, n)
        v[f"verb{n}"] = round(sum(x in ref["ng"][n] for x in g) / max(len(g), 1), 3)
    return {
        "words": len(w),
...
def main():
    ...
    ref = load_reference(ROOT / "data" / "wake_clean.txt")
    ...
    for _ in range(max(a.samples, 8)):
        ...
        d = describe(slice_, ref, vocab=rest)
        if d:
            book_stats.append(d)
